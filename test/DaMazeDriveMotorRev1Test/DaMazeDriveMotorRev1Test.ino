...
// Set initial speed to 70% of maximum.
...
int leftSpeed    = MAXSPEED * .70;
int rightSpeed   = MAXSPEED * .70;
...
void loop ( void ) {
  if (digitalRead(MOTOR_STOP_SW) == true) {   // is the "Brake" switch activated?
  allStop(); }
  else{
    if (digitalRead(MOTOR_REV_SW) == true) {   // is the "go in reverse" switch activated?
    reverse(); 
    Forward  = false; 
    delay(20);} 
    else {
...
// Responsibilities :: Motors Left pivot at (leftSpeed, 0)
  ...
  // Turning can be achieved by stopping the wheel(s) on one side...
...
void right ( void ) {
// Responsibilities :: Motors Right pivot at (0, rightSpeed)
